        ...
        $.fn.inputmask = function (fn, options) {
                    ...
                    if (activeMaskset['lastValidPosition'] == undefined || (isRTL || opts.numericInput) ? activeMaskset['lastValidPosition'] <= opts.numericInput ? getMaskLength(buffer) : seekNext(buffer, maskPos) : activeMaskset['lastValidPosition'] >= seekPrevious(buffer, maskPos)) {
                        if (maskPos >= 0 && maskPos < getMaskLength(buffer)) {
                            ...
                            if (results[index] !== false) {
                                ...
                                activeMaskset['lastValidPosition'] = isRTL ? pos == getMaskLength(buffer) ? undefined : seekNext(buffer, pos) : pos == 0 ? undefined : seekPrevious(buffer, pos); //autocorrect validposition from backspace etc  	
                        }
                    }
                });
            ...
            function mask(el) {
                ...
                if (getMaskLength(buffer) > maxLength && maxLength > -1) { //FF sets no defined max length to -1 
                    if (maxLength < getActiveBuffer().length)
                        getActiveBuffer().length = maxLength;
                    if (getActiveMaskSet()['greedy'] == false) {
                ...
                function keydownEvent(e) {
                    ...
                    if (k == opts.keyCode.BACKSPACE || k == opts.keyCode.DELETE || (iphone && k == 127)) {//backspace/delete
                        ...
                        setTimeout(function () {
                            ...
                            if (!opts.insertMode && caretPos == getMaskLength(buffer) && !e.shiftKey)
                                caretPos--;
                ...
                if (e.keyCode == opts.keyCode.TAB) {
                    ...
                    if (radixPosition != -1) {
                        ...
                        for (var i = 1; i <= opts.digits && i < opts.getMaskLength(masksets[activeMasksetIndex]["_buffer"], masksets[activeMasksetIndex]["greedy"], masksets[activeMasksetIndex]["repeat"], buffer, opts); i++) {
                            if (buffer[radixPosition + i] == undefined)
